...
// 返回逐像素 SSIM 图 [H, W]（RGB通道取均值），用于像素级筛选
inline torch::Tensor ssim_per_pixel(
    torch::Tensor &img1,
    torch::Tensor &img2,
    int window_size = 11)
{
    int64_t channel = img1.size(-3);
    auto window = create_window(window_size, channel, img1.device().type());
    window = window.type_as(img1);
    int half = window_size / 2;

    auto mu1 = torch::nn::functional::conv2d(img1, window,
        torch::nn::functional::Conv2dFuncOptions().padding(half).groups(channel));
    ...
    auto sigma1_sq = torch::nn::functional::conv2d(img1 * img1, window,
        torch::nn::functional::Conv2dFuncOptions().padding(half).groups(channel)) - mu1_sq;
    auto sigma2_sq = torch::nn::functional::conv2d(img2 * img2, window,
        torch::nn::functional::Conv2dFuncOptions().padding(half).groups(channel)) - mu2_sq;
    auto sigma12 = torch::nn::functional::conv2d(img1 * img2, window,
        torch::nn::functional::Conv2dFuncOptions().padding(half).groups(channel)) - mu1_mu2;
    auto C1 = 0.01f * 0.01f;
    auto C2 = 0.03f * 0.03f;
    return ((2.f * mu1_mu2 + C1) * (2.f * sigma12 + C2))
        / ((mu1_sq + mu2_sq + C1) * (sigma1_sq + sigma2_sq + C2)).mean(0);  // [H, W]
}
