...
protected:
    ...
    bool checkKFupdateFromGSRequested();
...
public:
    ...
    // SLAM system
    std::shared_ptr<dso::FullSystem> pSLAM_;
    ...
    float distortion_p1 = 0.0f;
    float distortion_p2 = 0.0f;
    float distortion_k3 = 0.0f;
    float depth_scale = 1000.0f;
    float lambda_sparse_depth = 0.0f;

    std::string loc_camera_cfg_path;
    ...
    // ablation study!!!!!!!!
    // std::chrono::steady_clock::time_point additional_training_start;
    // int additional_training_start_iter = 0;

protected:
    // Parameters
    // FastGS: per-KF latest high-gradient mask, replaced on re-sample
    std::map<std::size_t, torch::Tensor> kf_grad_masks_;
    ...
    std::map<std::size_t, float> kf_recent_loss_;
    std::set<std::size_t> filtered_fids_;            // KFs skipped by loss filter
    float loss_low_thresh_ = 0.035f;
    std::vector<std::size_t> kfid_in_dso_active_window;
    int kfid_training = 0;

    // ===== Multi-frame Bundle Adjustment =====
    struct BAObservation {
        int frame_id;    // keyframe fid containing this observation
        float u, v;      // observed pixel coords (photometric-verified for near, original for ref)
    };
    struct BAPoint {
        Eigen::Vector3d X_ref_cam;    // 3D in ref camera frame (set at accumulation, pose-invariant)
        unsigned long ref_frame_id;   // ref frame for X_ref_cam (invalid after merge)
        Eigen::Vector3d X_world;      // world coords (computed at BA time, used by solver)
        std::vector<BAObservation> obs;
    };
    std::vector<BAPoint> ba_points_;                    // accumulated points
    std::map<unsigned long, int> ba_frame_id_to_idx_;   // fid → BA index
    std::vector<unsigned long> ba_frame_fids_;          // BA index → fid
    int ba_accum_iter_ = 0;                             // iters since last BA run
    void runMultiFrameBA();                             // BA solver
    ...
    float mapping_fx, mapping_fy, mapping_cx, mapping_cy;
    bool need_distortion = false;
