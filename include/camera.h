...
//    inline cv::Point2f undistortPoint(const cv::Point2f& pt) const
//    {
//        // 如果没有畸变系数或全部为 0，直接返回原始点
//        if (dist_coeff_.empty() ||
//            (dist_coeff_.at<float>(0) == 0 && dist_coeff_.at<float>(1) == 0 &&
//             dist_coeff_.at<float>(2) == 0 && dist_coeff_.at<float>(3) == 0 &&
//             dist_coeff_.at<float>(4) == 0))
//            return pt;
//
//        std::vector<cv::Point2f> src = {pt};
//        std::vector<cv::Point2f> dst;
//        // 使用原始内参 K_ 和畸变系数，得到去畸变后的坐标（输出使用去畸变内参 K_new_）
//        cv::undistortPoints(src, dst, K_, dist_coeff_, cv::noArray(), K_new_);
//        return dst[0];
//    }
