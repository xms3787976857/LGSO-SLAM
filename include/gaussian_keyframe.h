...
public:
    ...
    // ========== 新增：存储当前帧中每个点的共视观测（与 sparse_depth 非零像素索引对应） ==========
    struct Observation {
        unsigned long target_fid;
        cv::Point2f observed_pixel;
    };
    std::vector<std::vector<Observation>> observations;   // 每个点对应一组观测，索引与稀疏点顺序一致

    // ========== 新增：存储每个稀疏点的像素坐标和深度值 ==========
    std::vector<cv::Point2f> point_pixels;   // 与 observations 索引对应
    std::vector<float> point_depths;         // 对应的深度值（米）
};
