...
namespace dso {
    ...
    LoopClosing::LoopClosing(FullSystem *fullsystem) :
            ...
            globalMap(fullsystem->globalMap), Hcalib(&fullsystem->Hcalib),
    ...
    void LoopClosing::InsertKeyFrame(std::shared_ptr<Frame> &frame) {
        std::unique_lock<mutex> lock(mutexKFQueue);
    ...
    bool LoopClosing::DetectLoop(std::shared_ptr<Frame> &frame) {
    ...
    bool LoopClosing::CorrectLoop(CalibHessian* Hcalib) {
        ...
        std::shared_ptr<Frame> pKF = candidateKF;
        ...
        if (nmatches < 10) {
            ...
            for (size_t k = 0; k < matches.size(); k++) {
                auto &m = matches[k];
                std::shared_ptr<Feature> &featKF = pKF->features[m.index2];
                std::shared_ptr<Feature> &featCurrent = currentKF->features[m.index1];

                if (featKF->status == Feature::FeatureStatus::VALID &&
                    featKF->point->status != Point::PointStatus::OUTLIER) {
                    // there should be a 3d point
                    // pt unused?
                    //std::shared_ptr<Point> &pt = featKF->point;
            ...
            // setup pose graph
            {
                ...
                std::unique_lock<mutex> lock(currentKF->mutexPoseRel);
    ...
    bool LoopClosing::ComputeOptimizedPose(std::shared_ptr<Frame> pKF, Sim3 &Scr, CalibHessian* Hcalib,
                                           Mat77 &H, float windowSize) {

        LOG(INFO) << "computing optimized pose" << endl;
        int TH_HIGH = 50;
        vector<std::shared_ptr<Frame>> activeFrames = fullSystem->GetActiveFrames();
        // make the idepth map
        ...
        for (std::shared_ptr<Frame> fh: activeFrames) {
            if (fh == currentKF) continue;
            for (std::shared_ptr<Feature> feat: fh->features) {
                if (feat->status == Feature::FeatureStatus::VALID &&
                    ...
                    if (ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN) {
                        ...
                        if (r->target != currentKF->frameHessian.get()) continue;
        ...
        for (auto &p: candidateFeatures) {
            ...
            for (size_t &k: indices) {
                std::shared_ptr<Feature> &feat = currentKF->features[k];
        ...
        for (size_t i = 0; i < matchedFeatures.size(); i++) {
            ...
            EdgeProjectPoseOnlySim3 *eProj = new EdgeProjectPoseOnlySim3(fxiG[0], fyiG[0], cxiG[0], cyiG[0], matchedPoints[i]);
