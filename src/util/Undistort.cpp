...
Undistort* Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename)
{
	...
	else if(std::sscanf(l1.c_str(), "RadTanK3 %f %f %f %f %f %f %f %f %f",
            &ic[0], &ic[1], &ic[2], &ic[3],
            &ic[4], &ic[5], &ic[6], &ic[7], &ic[8]) == 9)
    {
        u = new UndistortRadTanK3(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
...
ImageAndExposure* Undistort::undistort(const MinimalImage<T>* image_raw, float exposure, double timestamp, float factor) const
{
	...
	if (!passthrough)
	{
		...
		for(int idx = w*h-1;idx>=0;idx--)
		{
			...
			if (xx < 0 || yy < 0 || xxi < 0 || yyi < 0 || xxi + 1 >= wOrg || yyi + 1 >= hOrg)
				out_data[idx] = 0;
			else
			{
...
MinimalImageB3* Undistort::undistort_rgb(const MinimalImageB3* image_raw, float exposure, double timestamp, float factor)
{
	int wh = w*h;
	...
	// ImageAndExposure* result = new ImageAndExposure(w, h, timestamp);
	MinimalImageB3* result = new MinimalImageB3(w, h);
	// Vec3b* data = result->data;

	// if (!valid || exposure <= 0 || setting_photometricCalibration == 0) // disable full photometric calibration.
	// {
	// 	for(int i=0; i<wh;i++)
	// 	{
	// 		data[i][0] = factor*image_raw->data[i][0];
	// 		data[i][1] = factor*image_raw->data[i][1];
	// 		data[i][2] = factor*image_raw->data[i][2];
	// 	}
	// }
	// else
	// {
	...
	// 	for(int i=0; i<wh;i++)
	// 	{
	// 		data[i][0] = photometricUndist->G[image_raw->data[i][0]];
	// 		data[i][1] = photometricUndist->G[image_raw->data[i][1]];
	// 		data[i][2] = photometricUndist->G[image_raw->data[i][2]];
	// 	}
	...
	// 		for(int i=0; i<wh;i++)
	// 		{
	// 			data[i][0] *= photometricUndist->vignetteMapInv[i];
	// 			data[i][1] *= photometricUndist->vignetteMapInv[i];
	// 			data[i][2] *= photometricUndist->vignetteMapInv[i];
	// 		}
	...
	if (!passthrough)
	{
		Vec3b* out_data = result->data;
		Vec3b* in_data = image_raw->data;
		...
		for(int idx = w*h-1;idx>=0;idx--)
		{
			...
			else
			{
				...
				for (int c = 0; c < 3; c++)
				{
					out_data[idx][c] = xxyy * src[1 + wOrg][c] +
									(yy - xxyy) * src[wOrg][c] +
									(xx - xxyy) * src[1][c] +
									(1 - xx - yy + xxyy) * src[0][c];
				}
			}
		}
	...
	{
		memcpy(result->data, image_raw->data, sizeof(float)*w*h*3);
	}
...
UndistortRadTanK3::UndistortRadTanK3(const char* configFileName, bool noprefix)
{
    printf("Creating RadTan undistorter\n");
    ...
    if(noprefix)
        ...
        readFromFile(configFileName, 9,"RadTanK3 ");
}
UndistortRadTanK3::~UndistortRadTanK3()
{
}

void UndistortRadTanK3::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    // RADTAN: parsOrg 배열에 있는 파라미터 재정렬 (총 9개)
    float fx = parsOrg[0];
    float fy = parsOrg[1];
    float cx = parsOrg[2];
    float cy = parsOrg[3];
    float k1 = parsOrg[4];
    float k2 = parsOrg[5];
    float k3 = parsOrg[6];  // 새로 추가된 3차 방사 왜곡 계수
    float r1 = parsOrg[7];  // 기존의 tangential 계수 (p1)
    float r2 = parsOrg[8];  // 기존의 tangential 계수 (p2)
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // 미리 계산: 각 축의 제곱, 교차항, 및 반경제곱
        float mx2_u = ix * ix;
        float my2_u = iy * iy;
        float mxy_u = ix * iy;
        float rho2_u = mx2_u + my2_u;

        // Radial Distortion: 기존의 2차, 4차 항에 6차 항 (k3 * rho2^3) 추가
        float rad_dist_u = k1 * rho2_u + k2 * (rho2_u * rho2_u) + k3 * (rho2_u * rho2_u * rho2_u);

        // Tangential Distortion (접선 왜곡)
        float x_dist = ix + ix * rad_dist_u + 2.0f * r1 * mxy_u + r2 * (rho2_u + 2.0f * mx2_u);
        float y_dist = iy + iy * rad_dist_u + 2.0f * r2 * mxy_u + r1 * (rho2_u + 2.0f * my2_u);

        // 최종 좌표 변환: 다시 픽셀 좌표계로 복원
        float ox = fx * x_dist + cx;
        float oy = fy * y_dist + cy;
