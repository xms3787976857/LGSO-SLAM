...
public:
	...
	MinimalImageB3* undistort_rgb(const MinimalImageB3* image_raw, float exposure=0, double timestamp=0, float factor=1);
...
class UndistortRadTanK3 : public Undistort
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    UndistortRadTanK3(const char* configFileName, bool noprefix);
    ~UndistortRadTanK3();
    void distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const;
