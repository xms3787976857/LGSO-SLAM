...
#include "util/PointXYZIndexBW.h"
...
public:
	...
	void initLoopClosing(const std::string &vocPath);
	void addKeyFrameToLoopClosing(int kfId, FrameHessian* fh);
	void finalizeLoopClosing();
	...
	void setHcalib(const CalibHessian& newHcalib) {
		Hcalib = newHcalib;
    }
	...
	std::vector<FrameHessian*> keepKFHessians;

	FrameHessian* current_fh;

	bool isSave;

	bool isRunMapping() const {
		return runMapping;
    }
	...
	// with 2DGS
	std::mutex allKeyframeMutex;

    const std::vector<FrameHessian*>& getFrameHessians() const {
        return frameHessians;
    }

	// Covariance Insertion
	std::pair<Eigen::Matrix2f, bool> compute2DCovariance(const Eigen::Vector3f* dI_l, const std::pair<int, int>& cov_center);
	std::pair<Eigen::Matrix3f, bool> compute3DCovariance(
		const std::vector<Eigen::Matrix2f>& Sigma_Is, 
		const std::vector<Eigen::MatrixXf>& JWs);
	Eigen::MatrixXf computeCovProjection(const Sophus::SE3f& w2c_extrinsic, const Mat33f& camera_matrix, const Eigen::Vector3f& point_world);
	void drawEllipse(cv::Mat& image, const cv::Point& center, const Eigen::Matrix2f& covariance, const cv::Scalar& color, int thickness=1);
	void plotImagesWithCovariances(const std::vector<cv::Mat>& images,
	...
	void createImageFromIntensity(const Eigen::Vector3f* dI_l, int width, int height, cv::Mat& outputImage);
	Eigen::Matrix2f project3DTo2DCovariance(const Eigen::Matrix3f& cov_3d, const Eigen::MatrixXf& T_last);

	bool isLastimg;
	int lastKFIncomingId = -1;
	bool existNewKFToServe = false;

	std::vector<FrameHessian*> frameHessians;	// ONLY changed in marginalizeFrame and addFrame.
	boost::mutex mapMutex;

	std::vector<FrozenFrameHessian*> newframeHessians;

	std::vector<FrozenFrameHessian*> frameHessiansFrozen;	// synced frames with 2DGS
	std::vector<PointHessian*> gsPoints;  // GS path: canActivate+GN, skip density
	boost::mutex frozenMapMutex;

	bool callKFUpdateFromGS = false;
	bool isDoneKFUpdateFromGS = false;

	std::vector<FrameShell*> allFrameHistory;

	// Loop closing bridge (PIMPL - no LDSO headers exposed)
	std::unique_ptr<LoopClosingBridge> lcBridge;
 ...
 */
	...
	// 用于坐标去畸变
    cv::Mat camera_original_K_;
    cv::Mat camera_dist_coeff_;
    cv::Mat camera_new_K_;
    bool need_distortion_for_coords_;

    // 坐标去畸变（使用与 GaussianMapper 相同的相机参数）
    cv::Point2f undistortPoint(const cv::Point2f& pt) const;
};
}
