...
struct FrameHessian
{
	...
	ImageAndExposure* input_img; // only used for localization
	MinimalImageB3* kf_img;
	MinimalImage<unsigned short>* kf_depth;
	cv::Mat kf_sparse_depth;
	bool exist_gt_img;
...
struct PointHessian
{
	...
	float kf_gray;
	std::vector<float> kf_color; // BGR

	Eigen::Vector3f eigenvalues;  // 3x1
	Eigen::Matrix3f eigenvectors; // 3x3
...
// to be served to 2DGS mapper from DSO
struct FrozenFrameHessian
{
	...
	std::vector<float> rots;
	    // ========== 新增：共视观测信息 ==========
    struct Observation {
        int target_frame_id;   // 观测帧的 incoming_id
        float observed_u;      // 观测到的像素坐标 u
        float observed_v;      // 观测到的像素坐标 v
    };
    // 每个点对应一组观测，与 pointsInWorld 索引相同
    std::vector<std::vector<Observation>> observations;

    // 每个点在本帧中的像素坐标（与 pointsInWorld 索引一致）
    std::vector<float> points_u;
