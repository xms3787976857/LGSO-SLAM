...
MinimalImageB3* readImageRGB_8U_resized(std::string filename, int width, int height)
{
	...
	// resize
	cv::Mat resized;
	cv::resize(m, resized, cv::Size(width, height), 0, 0, cv::INTER_NEAREST);
	MinimalImageB3* img = new MinimalImageB3(resized.cols, resized.rows);
	memcpy(img->data, resized.data, 3*resized.rows*resized.cols);
	return img;
}
...
MinimalImage<unsigned short>* readImageDepth_16U(std::string filename) {
    ...
    if (m.type() != CV_16UC1) 
	{
        printf("cv::imread did something strange! Expected CV_16UC1.\n");
        return nullptr;
    }
...
{
    cv::Mat m = cv::imdecode(cv::Mat(numBytes, 1, CV_8U, data), cv::IMREAD_COLOR);
    if (m.empty())
    {
        printf("cv::imdecode could not read stream (%d bytes)! This may segfault.\n", numBytes);
        return 0;
    ...
    MinimalImageB3* img = new MinimalImageB3(m.cols, m.rows);
    memcpy(img->data, m.data, m.cols * m.rows * sizeof(Vec3b));
    return img;
}

MinimalImageB3* readStreamRGB_8U_resized(char* data, int numBytes, int width, int height)
{
    cv::Mat m = cv::imdecode(cv::Mat(numBytes, 1, CV_8U, data), cv::IMREAD_COLOR);
    if (m.empty())
    {
    ...
    MinimalImageB3* img = new MinimalImageB3(resized.cols, resized.rows);
    memcpy(img->data, resized.data, resized.cols * resized.rows * sizeof(Vec3b));
    return img;
}
