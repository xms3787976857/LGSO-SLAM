...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& accum_metric_counts,
	bool get_flag)
{
  ...
  // FastGS: per-Gaussian metric count accumulator
  torch::Tensor out_accum = accum_metric_counts;
  if (!accum_metric_counts.defined()) {
      out_accum = torch::zeros({P}, torch::TensorOptions().dtype(torch::kInt32).device(device));
  }
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		debug,
		metric_map.defined() ? metric_map.contiguous().data_ptr<int>() : nullptr,
		out_accum.defined() ? out_accum.contiguous().data_ptr<int>() : nullptr,
		get_flag);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, geomBuffer, binningBuffer, imgBuffer, out_accum);
}
...
// FastGS-style render with per-Gaussian metric accumulation
std::tuple<torch::Tensor, torch::Tensor>
RasterizeGaussiansWithMetricCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
	const torch::Tensor& colors,
	const torch::Tensor& opacity,
	...
	const torch::Tensor& metric_map)
{
  CHECK_INPUT(background); CHECK_INPUT(means3D); CHECK_INPUT(colors);
  CHECK_INPUT(opacity); CHECK_INPUT(scales); CHECK_INPUT(rotations);
  CHECK_INPUT(transMat_precomp); CHECK_INPUT(viewmatrix); CHECK_INPUT(projmatrix);
  CHECK_INPUT(sh); CHECK_INPUT(campos); CHECK_INPUT(metric_map);

  const int P = means3D.size(0);
  const int H = image_height, W = image_width;
  ...
  torch::Tensor out_accum = torch::zeros({P}, int_opts);
  ...
  if (P != 0) {
      ...
      CudaRasterizer::Rasterizer::forward(
          ...
          out_accum.contiguous().data_ptr<int>(),
          true);   // get_flag
  }
  return std::make_tuple(out_color, out_accum);
}
