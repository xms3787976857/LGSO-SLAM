    ...
    const float spatial_lr_scale)
{
    ...
    scales = scales.unsqueeze(scales_ndimension).repeat({1, 2});
    ...
    const float spatial_lr_scale)
{
    ...
    torch::Tensor fused_color = sh_utils::RGB2SH(new_colors);
    auto temp = this->max_sh_degree_ + 1;
    torch::Tensor features = torch::zeros(
        {fused_color.size(0), 3, temp * temp},
        torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    ...
    ///////////////////// GS init
    // knn
    torch::Tensor point_cloud_copy = new_point_cloud.clone();
    torch::Tensor dist2 = torch::clamp_min(distCUDA2(point_cloud_copy), 0.0000001);
    torch::Tensor scales = torch::log(torch::sqrt(dist2));
    auto scales_ndimension = scales.ndimension();
    scales = scales.unsqueeze(scales_ndimension).repeat({1, 2});

    // torch::Tensor scales = torch::log( 0.02f * torch::ones(
    //     {new_point_cloud.size(0), 2},
    //      torch::TensorOptions().device(device_type_)));
    ///////////////////// GS init
    
    torch::Tensor rots = torch::zeros({new_point_cloud.size(0), 4}, torch::TensorOptions().device(device_type_));
    rots.index({torch::indexing::Slice(), 0}) = 1;
    ...
    const float spatial_lr_scale)
{
    ...
    //// from dso
    torch::Tensor scales = torch::log(
        torch::from_blob(
            scales_vec.data(), {num_new_points, 2},
            torch::TensorOptions().dtype(torch::kFloat32)).to(device_type_));

    torch::Tensor rots = torch::from_blob(
                            rots_vec.data(), {num_new_points, 4},
                            torch::TensorOptions().dtype(torch::kFloat32)).to(device_type_);
    ////
...
void GaussianModel::increasePcd(std::vector<float> points, std::vector<float> colors, 
                                std::vector<float> scales_vec, std::vector<float> rots_vec, int iteration)
{
// auto time1 = std::chrono::steady_clock::now();
    assert(points.size() == colors.size());
    assert(points.size() % 3 == 0);
    ...
    int max_screen_size)
{
    ...
    if (max_screen_size) {
        ...
        auto max_scales = std::get<0>(this->getScalingActivation().max(/*dim=*/1));
        auto big_points_ws = max_scales > 0.1f * max_screen_size;
        auto small_points_ws = max_scales < 0.01f;

        // prune_mask = torch::logical_or(torch::logical_or(prune_mask, big_points_vs), big_points_ws);
        // prune_mask = torch::logical_or(prune_mask, big_points_ws);
        prune_mask = torch::logical_or(torch::logical_or(prune_mask, big_points_ws), small_points_ws);
    }
...
// ===================================================================
// FastGS-style multi-view consistent densification & pruning
// ===================================================================
void GaussianModel::densifyAndPruneFastGS(
    torch::Tensor& multi_view_mask,
    torch::Tensor& multi_view_count,
    float max_grad,
    float min_opacity,
    float extent,
    int max_screen_size)
{
    ...
    auto grad_qualifiers = torch::where(
        torch::frobenius_norm(grads, /*dim=*/-1) >= max_grad, true, false);

    auto clone_qualifiers = std::get<0>(torch::max(this->getScalingActivation(), /*dim=*/1))
                            <= percentDense() * extent;
    auto split_qualifiers = std::get<0>(torch::max(this->getScalingActivation(), /*dim=*/1))
                            > percentDense() * extent;

    auto all_clones = torch::logical_and(clone_qualifiers, grad_qualifiers);
    auto all_splits = torch::logical_and(split_qualifiers, grad_qualifiers);

    auto clone_selected = torch::logical_and(multi_view_mask, all_clones);
    auto split_selected = torch::logical_and(multi_view_mask, all_splits);

    // Clone
    {
        auto new_xyz = this->xyz_.index({clone_selected});
        auto new_features_dc = this->features_dc_.index({clone_selected});
        auto new_features_rest = this->features_rest_.index({clone_selected});
        auto new_opacities = this->opacity_.index({clone_selected});
        auto new_scaling = this->scaling_.index({clone_selected});
        auto new_rotation = this->rotation_.index({clone_selected});
        auto new_exist_since_iter = this->exist_since_iter_.index({clone_selected});
    ...
    // Split
    {
        int N = 2;
        int n_init_points = this->getXYZ().size(0);
        torch::Tensor padded = torch::zeros({n_init_points},
            torch::TensorOptions().dtype(torch::kBool).device(device_type_));
        padded.slice(0, 0, split_selected.size(0)).copy_(split_selected);

        auto stds = this->getScalingActivation().index({padded}).repeat({N, 1});
        stds = torch::cat({stds, torch::zeros({stds.size(0), 1},
            torch::TensorOptions().device(device_type_))}, /*dim=*/-1);
        auto means = torch::zeros({stds.size(0), 3}, torch::TensorOptions().device(device_type_));
        auto samples = at::normal(means, stds);
        auto r_masked = this->rotation_.index({padded});
        auto rots = general_utils::build_rotation(r_masked).repeat({N, 1, 1});
        auto new_xyz = torch::bmm(rots, samples.unsqueeze(-1)).squeeze(-1)
                     + this->getXYZ().index({padded}).repeat({N, 1});
        auto new_scaling = torch::log(
            this->getScalingActivation().index({padded}).repeat({N, 1}) / (0.8f * N));
        auto new_rotation = this->rotation_.index({padded}).repeat({N, 1});
        auto new_features_dc = this->features_dc_.index({padded}).repeat({N, 1, 1});
        auto new_features_rest = this->features_rest_.index({padded}).repeat({N, 1, 1});
        auto new_opacity = this->opacity_.index({padded}).repeat({N, 1});
        auto new_exist_since_iter = this->exist_since_iter_.index({padded}).repeat({N});
        ...
        auto prune_filter = torch::cat({
            padded, torch::zeros({(N * padded.sum()).item<int>()},
                torch::TensorOptions().device(device_type_).dtype(torch::kBool))
        });
        this->prunePoints(prune_filter);
    }

    // FastGS-style VCP: weighted random pruning by multi-view redundancy
    auto prune_mask = (this->getOpacityActivation() < min_opacity).squeeze();
    if (max_screen_size) {
        auto max_scales = std::get<0>(this->getScalingActivation().max(/*dim=*/1));
        prune_mask = torch::logical_or(prune_mask, max_scales > 0.1f * max_screen_size);
    }

    int to_remove = prune_mask.sum().item<int>();
    int remove_budget = (int)(0.5f * to_remove);

    if (remove_budget > 0) {
        // Weight: Gaussians seen by FEWER KFs are more redundant (higher weight to prune)
        auto scores = 1.0f / (multi_view_count.to(torch::kFloat32) + 1.0f);

        int n_init_points = this->getXYZ().size(0);
        auto padded_scores = torch::zeros({n_init_points},
            torch::TensorOptions().dtype(torch::kFloat32).device(device_type_));
        padded_scores.slice(0, 0, scores.size(0)).copy_(scores);
        padded_scores.index_put_({~prune_mask}, 0.0f);

        auto selected_mask = torch::zeros({n_init_points},
            torch::TensorOptions().dtype(torch::kBool).device(device_type_));
        auto sampled = torch::multinomial(padded_scores, remove_budget, /*replacement=*/false);
        selected_mask.index_put_({sampled}, true);

        auto final_prune = torch::logical_and(prune_mask, selected_mask);
        this->prunePoints(final_prune);
    }

    // Reset for next window
    this->xyz_gradient_accum_.zero_();
    this->denom_.zero_();
...
void GaussianModel::loadPly(std::filesystem::path ply_path)
{
    ...
    try { scales = ply_file.request_properties_from_element("vertex", { "scale_0", "scale_1" }); }
    catch (const std::exception & e) { std::cerr << "tinyply exception: " << e.what() << std::endl; }
